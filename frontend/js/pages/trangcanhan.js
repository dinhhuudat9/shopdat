...
window.pageInit = async function(params) {
    ...
    function syncTabCounts(stats = {}) {
        ...
        if (postsTabButton) {
            postsTabButton.innerHTML = `Bai dang <span class="tab-count">${posts}</span>`;
        }
    ...
    function renderProducts(items) {
        if (!items.length) {
            productsTab.innerHTML = renderEmptyTabState({
                ...
                description: 'Danh sach san pham se xuat hien tai day khi nguoi dung bat dau dang ban.'
            });
            return;
        }
            ...
            <div class="section-card profile-hero">
                ...
                <div class="profile-identity-strip profile-identity-strip-under-cover">
                    ...
                    <div class="profile-identity-text">
                        <h2 class="profile-name profile-name-dark">${renderDisplayName(user, user.email)}</h2>
                    </div>

                    <div class="profile-identity-actions">
                        ${isOwner ? `<button type="button" id="toggle-settings" class="btn-outline profile-identity-edit-btn">${settingsVisible ? 'Dong cai dat' : 'Chinh sua ho so'}</button>` : ''}
                    </div>
                </div>
            </div>
            ...
            <div class="profile-summary-strip">
                <div class="profile-stats profile-stats-standalone">
                    <div class="profile-stat">
                        <div class="profile-stat-label">Bai dang</div>
                        <div class="profile-stat-value">${stats.posts}</div>
                    </div>
                    <div class="profile-stat">
                        <div class="profile-stat-label">San pham</div>
            ...
            <div class="profile-about-grid">
                <div class="section-card profile-section">
                    <div class="profile-section-head">
                        <div>
                            <div class="profile-section-kicker">Giới thiệu</div>
                ...
                <div class="section-card profile-section">
                    <div class="profile-section-head">
                        <div>
                            <div class="profile-section-kicker">Kết nối</div>
                            <h3>Liên hệ</h3>
                        </div>
                    </div>
    ...
    function renderContactOverview(user, contactItems = []) {
        ...
        contactItems.slice(0, 3).forEach((item) => {
            cards.push({
                ...
                value: formatContactDisplay(item) || item.label || 'Dang cap nhat',
